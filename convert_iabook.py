...
def usage():
    sys.stderr.write("\n")
    sys.stderr.write("Usage: convert_iabook.py book_id path_to_book_files [out.expub]\n")
    sys.stderr.write("  Output defaults to book_id.expub.\n")
    sys.stderr.write("\n")
    sys.stderr.write("  -d calls expubcheck-1.0.3.jar to check output.\n")
    sys.stderr.write("  (expubcheck jar is assumed to be in the script directory)\n")
...
def main(argv):
    ...
    if make_daisy:
        ...
        alt_booktext = "This is a protected daisy format book.  If you are hearing this message, then your device is missing the appropriate key to read this book.  For more information, see the archivestream.github.io daisy faq."
#         iabook_to_daisy.process_book(iabook, ebook, alt_booktext)
        ...
        ebook = expub.Book(out_name, metadata)
        iabook_to_expub.process_book(iabook, ebook)
    ...
    if debug_output:
        if make_daisy:
            ...
            expubcheck = os.path.join(sys.path[0], 'expubcheck/expubcheck-1.1.jar')
            output = os.popen('java -jar ' + expubcheck + ' ' + out_name)
        print output.read()
