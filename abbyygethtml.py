...
def s3save(url,data):
    ...
    c=HTTPConnection("s3.us.archivestream.github.io")
...
def find_scandata(bookid,doc,path):
    ...
    if not path: path="http://www.archivestream.github.io/downloads/"
...
def find_abbyy(bookid,doc=False,path=False):
    if not doc: doc=bookid
    if not path: path="http://www.archivestream.github.io/download/"
    base=path+bookid+"/"+doc
...
def gethtml(spec,doc=False,
    ...
    if not skipcache and useS3:
        try:
            cachestream=urlopen(
                ("http://www.archivestream.github.io/download/%s/%s_abbyy.html"%(bookid,bookid)))
        except HTTPError:
            try:
                cachestream=urlopen(
                    ("http://www.archivestream.github.io/download/%s/%s_ondemand.html"%(bookid,bookid)))
            except HTTPError:
                cachestream=False
    if cachestream:
    ...
    if filecache and not skipcache:
        ...
        stream.close()
    if useS3:
        path="http://s3.us.archivestream.github.io/%s/%s_ondemand.html"%(bookid,bookid)
        path="http://s3.us.archivestream.github.io/abbyyhtml/%s_ondemand.html"%bookid
        try:
